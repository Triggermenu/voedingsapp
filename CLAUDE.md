...
- **Reikwijdte:** DB-breed uitgerold op alle categorieën met items boven de drempel — `zoetwaren` (37, incl. jam/honing/siroop; uitgezonderd hartige snacks zoals chips, laag-suiker gebak zoals croissant, en zoetstoffen zoals aspartaam/sucralose), gedroogd fruit (6: dadels, rozijnen, gedroogde vijg/abrikoos/pruim), zoete sauzen (4: ketchup, zoete chilisaus, ketjap manis, hoisinsaus) en gezoete granen (1: granola). **Bewust níét gevlagd:** vers fruit (heel fruit mét vezel — de bron-grondslag onderscheidt dit juist van vrije/vloeibare suiker) en gezoete zuivel die onder de drempel blijft (vla ~15 g, chocolademelk ~10 g). Suikergezoete dranken dragen géén vlag — die zijn al jicht-3 gescoord (§2.1).
...
| `primaryModulators` | `string[]` (max 3) | Slug-formaat `[a-z0-9-]`. Benoemt de voornaamste modulerende/mechanistische factoren achter de score. Mag bij **elk** triggerType voorkomen waar een concrete modulator bekend is (niet alleen `individueel-variabel`/`context-afhankelijk`): bv. `["ethanol","biogene-aminen"]` bij bier (`populatiebreed`), `["quercetine-3-glucuronide","ethanol","aldh2-variant"]` bij rode wijn (`subgroep-overschat`), `["nitriet","stikstofmonoxide"]` bij spek (`dosis-afhankelijk`), `["microbioom","nitraat-conversie"]` bij nitraatgroente (`individueel-variabel`). Het veld documenteert de biochemische actoren — nuttig voor diëtist-/onderzoekersuitleg. |
...
- **Wijzigingen:** alleen door Peter, met expliciete akkoordregistratie in commit message.
  - v3.1 (2026-06-12): **`highAddedSugar`-vlag DB-breed uitgerold (batch 2) — afronding van de §2.5-reikwijdte.** Na de eerste `zoetwaren`-batch (v3.0) de overige categorieën langs de FSA-grens (>22,5 g suiker/100 g) gelegd. **Gevlagd:** gedroogd fruit (6: dadels, rozijnen, gedroogde vijg/abrikoos/Turkse vijg/pruim), zoete sauzen (4: ketchup ~23 g, zoete chilisaus ~50 g, ketjap manis ~55 g, hoisinsaus ~30 g), gezoete granen (1: granola ~28 g). **Bewust níét gevlagd:** vers fruit (heel fruit mét vezel — de bron-grondslag onderscheidt dit juist van vrije/vloeibare suiker), gezoete zuivel die ónder de drempel blijft (vla ~15 g, chocolademelk ~10 g → 0 items), en bereid-gerecht (hartig, 0 items). Suikergezoete dranken dragen géén vlag — al jicht-3 gescoord (§2.1). §2.5-reikwijdte herschreven naar de definitieve dekking. Geen schema-/score-/drempelwijziging — uitsluitend data-uitbreiding van de bestaande vlag. Akkoord: Peter Wolterman (chat 2026-06-12, "volledig").
  ...
  - v2.6 (2026-06-04): Jicht- en histamine-as op migraine-niveau geborgd (CI-gates + flag-volledigheid). Geen score-/regelwijzigingen. **Histamine:** 8 liberator- + 12 daoBlocker-flags aangevuld op duidelijke gevallen (chocolade = liberator per §2.4, niet DAO-blokker; valse matches citroenverbena/ananaskers/wild-zwijn overgeslagen). Alle alcohol met histamine-score heeft nu daoBlocker. **CI-gates:** 13 (jicht single-ingredient note-purinewaarde moet bij §2.1-drempel passen — borgt de DB-brede note↔score-consistentie die handmatig was bereikt) + 14 (alcohol met histamine-score vereist daoBlocker-flag). Beide bewezen falend op overtreding. Resterend op nierstenen-as: per-portie-verificatie tegen Harvard (aparte focus-pass; notes citeren 93/115× per-100g terwijl drempel per-portie is). Akkoord: Peter Wolterman (chat 2026-06-04, keuze "1").
  - v2.5 (2026-06-04): Uitputtende 4-assen consistentie-pass (semantische note-sweep). Geen scorewijzigingen — uitsluitend note/flag/doc-consistentie + 2 nieuwe CI-gates. **Migraine:** 19 alcohol-`subgroep-overschat`-notes genormaliseerd (jenever/absint zeiden "sterke trigger" terwijl het label subgroep-overschat is — exact het spek-patroon; witte wijn/prosecco/likeur hadden zwakke notes). Nu uniform de congeneren/quercetine/subgroep-onderbouwing (Onderwater 2019). **Histamine:** citrus `omstreden`-voorbehoud toegevoegd aan 8 items (§2.4/§12-eis, stond alleen op Sinaasappel); liberator-flag gezet op 6 duidelijke liberatoren. **CI-gates:** 9c (migraine subgroep-/variabel-triggerType mag geen populatiebreed-claim in note hebben — vangt de note-label-tegenspraak) + 12 (citrus-histamine vereist omstreden-note). Beide bewezen falend op overtreding. **Doc:** §3.3 `primaryModulators` verbreed — veld wordt zinvol bij elk triggerType gebruikt (mechanistische actoren, 151 items), niet alleen individueel-variabel/context-afhankelijk. Audit-resultaat: jicht/nierstenen note-vs-score 0 tegenspraken, migraine note-label 0 tegenspraken, citrus 0 zonder omstreden. Akkoord: Peter Wolterman (chat 2026-06-04, "uitputtende pass alle vier"); definitieve accept/revert door Peter aan einde.
  - v2.4 (2026-06-04): Migraine-as consistent gemaakt met criterium 3 + nieuwe CI-gate. Audit toonde 3 gecureerd-vlees-items op score 3 met triggerType `subgroep-bevestigd` — strijdig met §2.2 criterium 3 (score 3 ⇒ populatiebreed/dosis-afhankelijk). **Spek + bacon → `dosis-afhankelijk`** (matcht hun Henderson-dosis-note + §2.2.1-voorbeeld; score 3 blijft). **Rookworst → score 2 + `subgroep-bevestigd`** (sub-Henderson, zoals salami/chorizo/paté; note beschreef al een subgroep-effect zonder dosisfiguur — Peters keuze "B"). Nieuwe **CI-gate 9b** in `scripts/validate-db.ts`: migraine score 3 vereist triggerType populatiebreed/dosis-afhankelijk — bewaakt criterium 3 automatisch. Resultaat: alle 8 migraine-score-3-items compliant (6 bier=populatiebreed, 2 spek=dosis-afhankelijk). Akkoord: Peter Wolterman (chat 2026-06-04, keuze "B").
  - v2.3 (2026-06-04): Jicht-as volledig consistent gemaakt (laatste afwijkingen) + amandelmelk-conflict opgelost. Na een complete DB-brede consistentiescan (robuuste parser voor `<`/`>`/decimaalkomma) resteerden 3 punten, alle evidence-gedreven rechtgetrokken: (1) **tamarinde** jicht 1→0 (<50 mg, vruchtvlees); (2) **gerookte zalm** jicht 2→3 — roken concentreert purine tot >200 mg/100g (~300), eerdere note (~170) was de verse-zalm-waarde; (3) **amandelmelk** nierstenen-conflict opgelost: oxalaat ~27–50 mg/kop (gespecialiseerde nierstenen-diëtistenbron) → §2.3-drempel score 2; zuivel-versie 1→2 gecorrigeerd, noten-zaden-duplicaat verwijderd (almond milk hoort als drank bij de plantaardige melken in zuivel). **Zalm in blik bewust ongewijzigd** (score 2): evidence tegenstrijdig (USDA 13 mg vs "vergelijkbaar met vers ~150"). Resultaat: 0 jicht note-vs-drempel-afwijkingen DB-breed, 0 cross-file duplicaten, 669 items. Geen nieuwe paradigma's. Akkoord: Peter Wolterman (chat 2026-06-04, "ik wil consequent zijn").
